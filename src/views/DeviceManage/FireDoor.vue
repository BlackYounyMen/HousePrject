<template >
  <div>
    ...
    <!--查询条件结束-->
    <div v-for="(item, index) in FireDoorlist" :key="index">
      <div
      ...
      >
        ...
        <div style="height: 100%; border: 1px solid #807e7e">
          <img
            src="../../Pic/微信图片_20230213200301.png"
            v-if="item1.DoorStatus == true"
            style="width: 100%; height: 75%"
          />
          <img
            src="../../Pic/微信图片_20230213200308.png"
            v-if="item1.DoorStatus == false"
            style="width: 100%; height: 75%"
          />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    ...mapMutations(["SET_FireDoor_INFO", "SET_STATE_UPDATE"]),
    //循环出echarts的图表
    GaugeBasic() {
    ...
    handleCurrentChange(val) {
      ...
      this.loadFireDoorData(this.Searth);
      this.GaugeBasic();
    },
    Searthitem() {
      this.loadFireDoorData(this.Searth);
    ...
    SeeItem() {
      this.ObtainSecondSelect(
        `https://localhost:5001/api/Device/GetFireUnitAll?id=${this.Searth.building}`
      );
    },
  },
  mounted() {
    this.GaugeBasic();
    this.ObtainSelect("https://localhost:5001/api/Device/GetFireAll");
  },
  created() {
    this.loadFireDoorData(this.Searth);
  },
