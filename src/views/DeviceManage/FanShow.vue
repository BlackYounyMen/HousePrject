    ...
    <div v-for="(item, index) in Fanlist" :key="index">
      ...
      >
        ...
        <div style="height: 100%; border: 1px solid #807e7e">
          <img src="../../Pic/fengji.png" style="width: 250px; height: 250px" />
        </div>
      </div>
    </div>
    <footer>
...
export default {
  ...
  methods: {
    ...
    ...mapMutations(["SET_Fan_INFO", "SET_STATE_UPDATE"]),
    //循环出echarts的图表
    GaugeBasic() {
    ...
    SeeItem() {
      this.ObtainSecondSelect(
        `https://localhost:5001/api/Device/GetFanUnitAll?id=${this.Searth.building}`
      );
    },
  },
  mounted() {
    this.GaugeBasic();
    this.ObtainSelect("https://localhost:5001/api/Device/GetFanAll");
  },
  created() {
    this.loadFanData(this.Searth);
  },
