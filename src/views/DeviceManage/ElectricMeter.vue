<template >
  <div>
    ...
    <!--查询条件结束-->
    <div v-for="(item, index) in ElectricMeterlist" :key="index">
      <div
...
export default {
  ...
  methods: {
    ...mapActions([
      "loadElectricMeterData",
      "EditState",
      "ObtainSelect",
      "ObtainSecondSelect",
    ]),
    ...mapMutations(["SET_ElectricMeter_INFO", "SET_STATE_UPDATE"]),
    //循环出echarts的图表
    GaugeBasic() {
    ...
    handleCurrentChange(val) {
      ...
      this.loadElectricMeterData(this.Searth);
      this.GaugeBasic();
    },
    Searthitem() {
      this.loadElectricMeterData(this.Searth);
    ...
    SeeItem() {
      this.ObtainSecondSelect(
        `https://localhost:5001/api/Device/GetElectricUnitAll?id=${this.Searth.building}`
      );
    },
  },
  mounted() {
    this.GaugeBasic();
    this.ObtainSelect("https://localhost:5001/api/Device/GetElectricAll");
  },
  created() {
    this.loadElectricMeterData(this.Searth);
  },
